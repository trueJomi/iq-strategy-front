...
export async function sendMessage( data: DataFromModel<ContextFromIndividualModel>) {
    ...
    const fun = httpsCallableFromURL(funtions, 'https://sendmessage-qizpphcqca-uc.a.run.app')
...
export async function getPrediction(data: ContextPrediction) {
    ...
    const fun = httpsCallableFromURL(funtions, 'https://getpredcitions-qizpphcqca-uc.a.run.app')
    const result = await fun(data)
